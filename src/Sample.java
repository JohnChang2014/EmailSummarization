...
public class Sample {
	...
	private void initializeGate() throws GateException, MalformedURLException {
		...
		System.setProperty("gate.user.config", System
				.getProperty("gate.site.config"));
		...
		Gate.getCreoleRegister()
				.registerDirectories(new File(Gate.getGateHome().toString() + fileSeparator + "plugins" + fileSeparator + "ANNIE")
						.toURL());
		Gate.getCreoleRegister()
				.registerDirectories(new File(Gate.getGateHome().toString() + fileSeparator + "plugins" + fileSeparator + "Tools")
						.toURL());
		...
		_annieController = (SerialAnalyserController) Factory
				.createResource("gate.creole.SerialAnalyserController", Factory
						.newFeatureMap(), Factory.newFeatureMap(), "ANNIE_" + Gate
						.genSym());
		...
		pr = (ProcessingResource) Factory
				.createResource("gate.creole.POSTagger", params);
		_annieController.add(pr);
		System.err.println("Loaded: POS tagger");

		params = Factory.newFeatureMap();
		URL u = new File("ie/deri/nlp/LanguageResources/gazetteers/mylists.def")
				.toURL();
		...
		System.err
				.println("Loaded: Relation Extraction JAPE grammars, using JAPE Transducer");

	}

	public static void main(String args[]) {

		Sample _tutorial = new Sample();
		try {
	...
	private void extractRelations(File[] files) throws Exception {
		...
		for (int i = firstFile; i < files.length; i++) {
			...
			if (file.isFile() && file.length() != 0) {
				...
				AnnotationSet companytoCompany1 = defaultAnnotSet
						.get("companytoCompanyRelation1");

				if (!companytoCompany1.isEmpty()) {
					toRDF(companytoCompany1, 1);
				}
				AnnotationSet companytoCompany1a = defaultAnnotSet
						.get("companytoCompanyRelation1a");
				if (!companytoCompany1a.isEmpty()) {
					toRDF(companytoCompany1a, 7);
				}
				AnnotationSet companytoCompany2 = defaultAnnotSet
						.get("companytoCompanyRelation2");
				...
				System.out
						.print("Document " + file + " is empty! Will ignore and continue to next file!");
			}
		}// endfor
	...
	private void toRDF(AnnotationSet relationAnnotationSet, int code) {
		...
		while (annoIter.hasNext()) {
			...
			try {
				subject = doc
						.getContent()
						.getContent(subjectAnn.getStartNode().getOffset(), subjectAnn
								.getEndNode().getOffset()).toString();
			...
			try {
				object = doc
						.getContent()
						.getContent(objectAnn.getStartNode().getOffset(), objectAnn
								.getEndNode().getOffset()).toString();
			} catch (InvalidOffsetException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
			if (subject.equals("empty") || predicate.equals("empty") || object
					.equals("empty")) {
				System.err
						.println("Null triples + " + subject + " " + predicate + " " + object);
			}

			if (subject.equals(object)) {
				System.err
						.println("Duplicate triples + " + subject + " " + predicate + " " + object);
			} else {
...

				{
					...
					System.out
							.print(subjectID + " " + predicateID + " " + objectID + " ." + newline);
